...
export function TokenUsageWidget() {
    ...
    if (authLoading || !user || !mounted) return null;
            ...
            <div className="bg-background/80 backdrop-blur-sm border border-border/50 rounded-xl p-4 shadow-lg">
                <div className="flex items-center justify-between text-sm">
                    <div className="flex items-center gap-2">
                        ...
                        <span className="text-primary-text-faded font-medium">
                            Credits
                        </span>
                        <button
                        ...
                        >
                            <RefreshCw
                                className={`w-3 h-3 text-primary-text-faded ${
                    ...
                    <div className="flex items-center gap-2">
                        <div className="text-right">
                            <div className="font-semibold text-foreground">
                                {formatCredits(
                                    tokensToCredits(tokenInfo.remainingTokens)
                                )}
                            </div>
                            <div className="text-xs text-primary-text-faded">
                                of{" "}
                                {formatCredits(
                                    tokensToCredits(tokenInfo.totalTokens)
                                )}
                            </div>
                        </div>
                        <CircularProgress
                            percentage={tokenInfo.usagePercentage}
                        />
                    </div>
                </div>
            </div>
